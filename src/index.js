...
async function botForKey(env, key) {
  ...
  return env.DB.prepare(`
    SELECT c.id,c.user_id,c.name,c.business_name,c.website_url,c.status,c.public_key,c.vector_store_id,
           ...
           COALESCE(NULLIF(s.plan_code,'starter'),'free') AS plan_code,
...
function conversationLimitForPlan(planCode) {
  const plan = String(planCode || "free").toLowerCase();
  if (plan === "enterprise") return 5000;
  if (["growth", "grow", "pro", "professional", "business"].includes(plan)) return 1000;
...
function authorizeBrowser(request, bot) {
  ...
  const ownOrigin = new URL(request.url).origin;
  const plan = String(bot.plan_code || "free").toLowerCase();
  if (["free", "starter"].includes(plan) && origin !== ownOrigin) return { ok: false, origin };
...
const requestedStyles = html`
  ...
  .contact-reference { padding:92px 0 125px; background:#f7f9fb; }
  .contact-layout { display:grid; grid-template-columns:minmax(0,.82fr) minmax(420px,1.18fr); gap:72px; align-items:start; }
  .contact-copy h1 { margin:12px 0 22px; font-size:clamp(46px,5vw,70px); line-height:1.04; letter-spacing:-.045em; }
  .contact-copy>p { max-width:620px; color:#5d6878; font-size:18px; line-height:1.7; }
  .contact-points { display:grid; gap:14px; margin-top:34px; }
  .contact-point { padding:17px 19px; border:1px solid #e0e6ec; border-radius:14px; background:#fff; }
  .contact-point strong { display:block; margin-bottom:4px; font-size:15px; }
  .contact-point span { color:#687486; font-size:14px; line-height:1.5; }
  .contact-form-card { padding:34px; border:1px solid #dfe5eb; border-radius:21px; background:#fff; box-shadow:0 18px 45px rgba(20,43,72,.07); }
  .contact-form-card h2 { margin:0 0 7px; font-size:27px; letter-spacing:-.025em; }
  .contact-form-card>p { margin:0 0 22px; color:#687486; line-height:1.55; }
  .contact-form-card label { display:block; margin:16px 0 7px; color:#26364a; font-size:13px; font-weight:800; }
  .contact-form-card input,.contact-form-card textarea { width:100%; padding:13px 14px; border:1px solid #cfd8e2; border-radius:11px; color:#071126; background:#fff; outline:none; }
  .contact-form-card textarea { min-height:145px; resize:vertical; }
  .contact-form-card input:focus,.contact-form-card textarea:focus { border-color:#159fbc; box-shadow:0 0 0 3px rgba(21,159,188,.12); }
  .contact-form-card .reference-button { width:100%; margin-top:20px; cursor:pointer; }
  .contact-notice { margin:0 0 20px; padding:13px 15px; border-radius:11px; font-size:14px; line-height:1.5; }
  .contact-notice.ok { border:1px solid #a9d9bd; color:#167044; background:#effaf3; }
  .contact-notice.error { border:1px solid #edb5b5; color:#a52b2b; background:#fff3f3; }
  .contact-honeypot { position:absolute!important; left:-10000px!important; width:1px!important; height:1px!important; overflow:hidden!important; }
  @media(max-width:900px){.contact-layout{grid-template-columns:1fr;gap:38px}.contact-reference{padding:65px 0 90px}}
  @media(max-width:560px){.contact-form-card{padding:24px 19px}}
...
const CONTACT_FORM_RECIPIENT = "sebslabbert1@gmail.com";

function referenceContact(c, state = {}) {
  const notice = state.message
    ? html`<div class="contact-notice ${state.error ? "error" : "ok"}" role="status">${escapeWebsiteHtml(state.message)}</div>`
    : "";
  return referenceShell(
    "Contact Fise AI",
    "Contact Fise AI about your chatbot, account, pricing or support requirements.",
    html`<main class="contact-reference"><div class="video-container"><div class="contact-layout"><section class="contact-copy"><div class="reference-eyebrow">Contact Fise AI</div><h1>How can we help?</h1><p>Tell us what you need and we’ll send your enquiry to the right person. You can ask about setup, pricing, your account, or using Fise AI on your website.</p><div class="contact-points"><div class="contact-point"><strong>Clear, practical support</strong><span>Describe the problem or goal in your own words.</span></div><div class="contact-point"><strong>Business enquiries</strong><span>Ask about plans, demonstrations, or a tailored rollout.</span></div><div class="contact-point"><strong>Account assistance</strong><span>Include the email address connected to your Fise account.</span></div></div></section><form class="contact-form-card" method="post" action="/contact"><h2>Send an enquiry</h2><p>Complete the short form and we’ll get back to you as soon as possible.</p>${notice}<label for="contact-name">Name</label><input id="contact-name" name="name" maxlength="100" autocomplete="name" required><label for="contact-email">Email address</label><input id="contact-email" name="email" type="email" maxlength="254" autocomplete="email" required><label for="contact-business">Business name <span style="font-weight:500;color:#7b8796">(optional)</span></label><input id="contact-business" name="business" maxlength="120" autocomplete="organization"><label for="contact-query">How can we help?</label><textarea id="contact-query" name="query" maxlength="3000" required></textarea><label class="contact-honeypot" aria-hidden="true">Website<input name="website" tabindex="-1" autocomplete="off"></label><button class="reference-button dark" type="submit">Send enquiry <span>→</span></button></form></div></div></main>`,
    c,
  );
}

async function submitContactForm(request, env, c) {
  if (!sameOrigin(request)) return response(referenceContact(c, { message: "Please refresh the page and try again.", error: true }), 403);
  if (!env.RESEND_API_KEY) return response(referenceContact(c, { message: "Email delivery is temporarily unavailable. Please try again shortly.", error: true }), 503);
  const form = await request.formData();
  if (String(form.get("website") || "").trim()) return response(referenceContact(c, { message: "Thank you. Your enquiry has been received." }));
  const name = String(form.get("name") || "").trim().replace(/[\r\n]+/g, " ").slice(0, 100);
  const email = String(form.get("email") || "").trim().toLowerCase().slice(0, 254);
  const business = String(form.get("business") || "").trim().replace(/[\r\n]+/g, " ").slice(0, 120);
  const query = String(form.get("query") || "").trim().slice(0, 3000);
  if (!name || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || query.length < 5) {
    return response(referenceContact(c, { message: "Please enter your name, a valid email address, and your enquiry.", error: true }), 400);
  }
  const sentAt = new Date().toISOString();
  const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(email + "|" + query + "|" + sentAt.slice(0, 13)));
  const idempotencyKey = [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, "0")).join("");
  const emailResponse = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { authorization: "Bearer " + env.RESEND_API_KEY, "content-type": "application/json", "idempotency-key": "fise-contact-" + idempotencyKey },
    body: JSON.stringify({
      from: "Fise AI <leads@fise.get-found.co.za>",
      to: [CONTACT_FORM_RECIPIENT],
      reply_to: email,
      subject: "Fise AI website enquiry from " + name,
      text: "Name: " + name + "\nEmail: " + email + "\nBusiness: " + (business || "Not provided") + "\n\nEnquiry:\n" + query + "\n\nSent: " + sentAt,
    }),
  });
  if (!emailResponse.ok) {
    console.error("Contact email delivery failed", emailResponse.status);
    return response(referenceContact(c, { message: "Your enquiry could not be sent right now. Please try again shortly.", error: true }), 502);
  }
  return go("/contact?sent=1");
}
...
async function handlePublicWebsite(request, env) {
  ...
  const { content: c } = await readWebsiteContent(env);
  if (url.pathname === "/contact" && request.method === "POST") return submitContactForm(request, env, c);
  ...
  if (url.pathname === "/contact") return response(referenceContact(c, url.searchParams.get("sent") === "1" ? { message: "Thank you. Your enquiry has been sent successfully." } : {}));
  ...
  return response(referenceContact(c));
}
...
const sharedStyles = html`
  ...
  .install-block{margin-top:12px}.install-block>small{display:block;margin-bottom:7px;color:#637083;font-weight:750}.install-block .embed-code{margin:0}.install-locked{padding:15px;border:1px solid #f0d3a3;border-radius:12px;background:#fffaf0}.install-locked strong{display:block;margin-bottom:5px;color:#7a4b00}.install-locked p{margin:0 0 12px;color:#687486;font-size:13px;line-height:1.5}.install-locked .btn{min-height:38px;padding:0 13px;font-size:12px}.conversation-usage{margin:15px 0 17px;padding:13px;border:1px solid #dfe6ef;border-radius:12px;background:#f8fafc}.conversation-usage-head{display:flex;align-items:center;justify-content:space-between;gap:12px}.conversation-usage-head strong{font-size:14px}.conversation-usage-head span,.conversation-usage small{color:#738094;font-size:11px}.conversation-usage-bar{height:7px;margin:10px 0 8px;overflow:hidden;border-radius:999px;background:#e4ebf3}.conversation-usage-bar span{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#20c6d8,#1769e0)}
...
function planConversationLimit(planCode) {
  const plan = String(planCode || "free").toLowerCase();
  if (plan === "enterprise") return 5000;
  if (["growth", "grow", "pro", "professional", "business"].includes(plan)) return 1000;
  if (plan === "essential") return 250;
        ...
        .map(
            ...
            (() => {
              ...
              <div class="bot-top">
                <div class="bot-identity">
                  ...
                  <div>
                    <div class="bot-name-row"><h2>${escapeHtml(bot.name)}</h2><span class="plan-badge">${escapeHtml(bot.plan_code || "free")}</span></div>
          ...
          bot.status === "ready"
            ? html` <div class="widget-tools">
                ...
                <div class="conversation-usage"><div class="conversation-usage-head"><strong>${conversationsRemaining.toLocaleString()} conversations remaining</strong><span>${conversationLimit.toLocaleString()} monthly</span></div><div class="conversation-usage-bar" aria-label="${conversationsRemaining} conversations remaining"><span style="width:${usagePercent}%"></span></div><small>Usage resets at the start of each month.</small></div>
            ...
            : html`<div class="widget-tools pending-tools"><div class="scan-label">Next step</div><h3>Complete the website scan</h3><p>Once your website knowledge is ready, you can customise and preview your chatbot here.</p><div class="locked-step">Chatbot tools unlock after scanning</div></div>`
              ...
              <details class="advanced-details">
                ...
                <div class="advanced-body">
                  ...
                  ${bot.status === "ready" ? (isFreePlan ? html`<div class="install-block install-locked"><small>Website installation code</small><strong>Upgrade to add this chatbot to your website.</strong><p>The Free plan lets you build and preview your chatbot, but website installation code is available on paid plans.</p><a class="btn" href="/#pricing">View plans</a></div>` : html`<div class="install-block"><small>Website installation code</small><code class="embed-code">&lt;script src=&quot;${escapeHtml(platformOrigin)}/widget.js?v=20260829-support-1&quot; data-chatbot-key=&quot;${escapeHtml(bot.public_key)}&quot;&gt;&lt;/script&gt;</code></div>`) : ""}
...
async function createEmailSession(email, env, destination = "/?signed_in=1") {
  ...
  if (!user) {
    ...
    await env.DB.batch([
      ...
      env.DB.prepare(
        "INSERT INTO subscriptions (id,user_id,provider,plan_code,status,created_at,updated_at) VALUES (?,?,'manual','free','active',?,?)",
...
async function showDashboard(request, env) {
  ...
  const usageMonthStart = new Date(Date.UTC(monthStart.getUTCFullYear(), monthStart.getUTCMonth(), 1)).toISOString();
  const result = await env.DB.prepare(
    `
