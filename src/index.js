    ...
    root.innerHTML = `
      <style>
        ...
        *{box-sizing:border-box}
        ...
        @media(max-width:620px){.panel.standard,.panel.large,.panel.fullscreen{inset:8px;width:auto;height:auto;max-height:none;transform:none;border-radius:18px}.question-grid,.large .question-grid,.fullscreen .question-grid{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}.head{min-height:96px;padding:42px 52px 8px 10px}.generic-chat-icon{width:36px;height:36px}.head-tools{right:7px;gap:1px}.head-control{width:33px}.callout{right:14px;bottom:96px}.launcher{right:14px;bottom:14px;width:64px;height:64px}.lead-grid{grid-template-columns:1fr}.lead-grid .wide{grid-column:auto}.messages{padding:13px}.bubble{max-width:94%}}
        @media(max-width:430px){.panel.standard,.panel.large,.panel.fullscreen{inset:4px;border-radius:15px}.questions{padding:8px 10px 10px}.question-grid,.large .question-grid,.fullscreen .question-grid{grid-template-columns:minmax(0,1fr)}.question:nth-child(n+5){display:none}.identity{justify-content:flex-start;gap:7px}.generic-chat-icon{width:34px;height:34px}.head strong{font-size:13px}.head small{max-width:150px}.messages{padding:11px}.composer-wrap{padding:8px 8px 5px}.quick-menu{right:-39px}}
      </style>
      <div class="callout">Need help with anything? 👋</div>
      ...
      <section class="panel ${safe(config.default_size || "standard")}" aria-label="Chat with ${safe(config.name)}">
        <header class="head">
          ...
          <div class="head-tools">
            <button class="head-control history-trigger" type="button" title="Chat History" aria-label="Chat History"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="6" cy="6.5" r="2.5"/><circle cx="18" cy="17.5" r="2.5"/><path d="M8.2 5.2C12 4 16 5 16 8.5V15c0 1.7 1 2.5 2 2.5"/><path d="M15.8 18.8C12 20 8 19 8 15.5V9c0-1.7-1-2.5-2-2.5"/></svg></button>
